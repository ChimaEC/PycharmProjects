def find_next_empty(puzzle):
    ...
    return None, None # if no spaces in the puzzle are
...
def is_valid(puzzle, guess, row, col): 
    ...
    row_start = (row // 3) * 3 # the whole number * 3 
    col_start = (col // 3) * 3
...
def solve_sudoku(puzzle):
    # solve puzzle using a backtracking
    # or puzzle is a list of lists, where each inner list is a row in our sudoku puzzle
    ...
    # step 1.1: if there's nowhwere left, then we're done because we only allowed valid inputs
    if row is None:
        return True

    # step 2: if there is a place to put a number, then make a guess btwn 1 and 9
